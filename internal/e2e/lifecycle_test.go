...
func TestLifecycleEndToEnd(t *testing.T) {
	...
	previewApply(t, []string{"acquire", "--workspace", dir, "--worker-id", "worker-a", "--session-id", "session-a", "--agent", "Agent A", "--purpose", "phase two", "--format", "json"})
	lifecycleCommand(t, []string{"resume", "--workspace", dir, "--worker-id", "worker-a", "--session-id", "session-a", "--generation", "1", "--format", "json"}, 0)
	lifecycleCommand(t, []string{"resume", "--workspace", dir, "--worker-id", "worker-b", "--session-id", "session-b", "--generation", "1", "--format", "json"}, 0)
	context := filepath.Join(t.TempDir(), "context.md")
	os.WriteFile(context, []byte("# Current\n\nReady.\n"), 0o600)
	previewApply(t, []string{"sync", "--workspace", dir, "--worker-id", "worker-a", "--session-id", "session-a", "--generation", "1", "--context-file", context, "--format", "json"})
	previewApply(t, []string{"checkpoint", "--workspace", dir, "--worker-id", "worker-a", "--session-id", "session-a", "--generation", "1", "--milestone-id", "phase-2", "--label", "Phase 2", "--format", "json"})
	previewApply(t, []string{"handoff", "--workspace", dir, "--worker-id", "worker-a", "--session-id", "session-a", "--generation", "1", "--purpose", "handoff", "--context-file", context, "--format", "json"})
