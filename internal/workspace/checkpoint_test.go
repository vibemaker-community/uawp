...
func TestPlanCheckpointCreatesImmutableSnapshot(t *testing.T) {
	...
	req := CheckpointRequest{WorkerID: "worker-a", MilestoneID: "phase-2", Label: "Phase 2 ready", DecisionReferences: []string{"DEC-001"}, At: at.Add(time.Hour)}
...
func TestCheckpointNameAndAuthorization(t *testing.T) {
	...
	for _, id := range invalid {
		if _, err := PlanCheckpointAt(root, CheckpointRequest{WorkerID: "worker-a", MilestoneID: id, Label: "label", At: at}); err == nil {
			t.Fatalf("accepted %q", id)
		}
	}
	if _, err := PlanCheckpointAt(root, CheckpointRequest{WorkerID: "worker-b", MilestoneID: "valid", Label: "label", At: at}); err == nil {
