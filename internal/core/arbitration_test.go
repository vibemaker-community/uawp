...
func TestRecoverStaleOwnershipReleasesOldClaim(t *testing.T) {
	...
	current := Ownership{Status: Active, WorkerID: "worker-a", Agent: "Agent A", AcquiredAt: at.Add(-time.Hour), Purpose: "work"}
	next, audit, err := RecoverStaleOwnership(current, RecoveryRequest{ControllerID: "human-1", Reason: "worker crashed", At: at})
	if err != nil {
		t.Fatal(err)
	}
	if next.Status != Released || next.WorkerID != "worker-a" || next.Agent != "Agent A" || next.ReleasedAt == nil {
		t.Fatalf("next=%#v", next)
	}
	if audit.ControllerID != "human-1" || audit.OldWorkerID != "worker-a" {
...
func TestRecoverStaleOwnershipRejectsInvalidRequest(t *testing.T) {
	at := time.Now()
	active := Ownership{Status: Active, WorkerID: "w", Agent: "a", AcquiredAt: at.Add(-time.Hour), Purpose: "p"}
